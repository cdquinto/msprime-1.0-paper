...
\author[24]{Hugo van Kemenade}
\author[7,25]{Anthony W. Wohns}
\author[7]{Yan Wong}
...
\author[10,27,$\dagger$]{Peter L. Ralph}
...
% Graham
\affil[4]{Lundbeck GeoGenetics Centre, Globe Institute, University of Copenhagen, 1350 Copenhagen K, Denmark}
% Aaron
\affil[5]{Department of Integrative Biology, University of Wisconsin--Madison, WI 53706, USA}
% Georgia
\affil[6]{Melbourne Integrative Genomics, School of Mathematics and Statistics, University of Melbourne, Victoria, 3010, Australia}
...
% Bjarki
\affil[8]{Leibniz Institute for Evolution and Biodiversity Science, Museum f\"ur Naturkunde Berlin, 10115, Germany }
% Castedo
\affil[9]{Fresh Pond Research Institute, Cambridge, MA 02140, USA}
% Jared, Murillo, Peter, Andy
\affil[10]{Institute of Ecology and Evolution, Department of Biology, University of Oregon, OR 97403-5289, USA}
% Jared
\affil[11]{Computational Biology Program, Fred Hutchinson Cancer Research Center, Seattle, WA 98102, USA}
% Ariella
\affil[12]{Department of Genetics, University of North Carolina at Chapel Hill, NC 27599-7264, USA}
\affil[13]{Embark Veterinary, Inc., Boston, MA 02111, USA}
% Gregor
\affil[14]{The Roslin Institute and Royal (Dick) School of Veterinary Studies, University of Edinburgh, EH25 9RG, UK}
% Bing
\affil[15]{Institute for Genome Sciences, University of Maryland School of Medicine, Baltimore, MD, 21201, USA}
% Winni
\affil[16]{Center for Hematology and Regenerative Medicine, Karolinska Institute, 141 83 Huddinge, Sweden}
% Michael
\affil[17]{Natural History Museum, University of Oslo, Blindern 0318 Oslo, Norway}
% Simon, Dom, Ivan
\affil[18]{Department of Human Genetics, McGill University, Montr\'eal, QC H3A 0C7, Canada}
% Nathaniel
\affil[19]{Department of Entomology, Pennsylvania State University, PA 16802, USA}
% Consuelo
\affil[20]{National Laboratory of Genomics for Biodiversity (LANGEBIO), Unit of Advanced Genomics, CINVESTAV, Irapuato 36821, Mexico}
% Saunack
\affil[21]{IIT Bombay, Powai, Mumbai 400 076, Maharashtra, India}
% Thibaut
\affil[22]{Professorship for Population Genetics, Department of Life Science Systems, Technical University of Munich, 85354 Freising, Germany}
% Kevin
\affil[23]{Ecology and Evolutionary Biology, University of California, Irvine, CA 92697, USA}
% Hugo
\affil[24]{No affiliation}
% Wilder 2
\affil[25]{Broad Institute of MIT and Harvard, Cambridge, MA 02142, USA}
% Jere
...
% \equalcontrib{1}
% \equalcontrib{2}
\equalcontrib{$\star$} % {Denotes shared first authorship, listed alphabetically}
\equalcontrib{$\dagger$} % {Denotes shared senior authorship, listed alphabetically}
...
see the \nameref{sec-arg} section below for
more details.
...
for $n$ samples.
As discussed in the \nameref{app-time-complexity} appendix, the quadratic
...
\newcommand\subfigtextsize{\small}
...
nodelabel/.style={font=\subfigtextsize}}
...
\centering
\caption*{(B)}
\begin{tikzpicture}[xscale=0.8, yscale=0.2, node distance=2mm and 10mm]
...
% Axes
\draw[very thick] ($(s0) + (0,-2.5)$) -- ($(s2r) + (0,-2.5)$);
\node (topAx) at (0,-1.0) {};
\node (topLeft) at ($(s0) + (0,-2.5)$) {};
\node (genUnit) at ($0.1*(s2r) - 0.1*(s0)$) {};
\foreach \x in {0, 5, 10} \draw ($(topLeft) + \x*(genUnit) + (0,0.1)$) -- +(0, -0.2); % tick marks
\node[anchor=north] at ($(topLeft)$) {\subfigtextsize 0.0};
\node[anchor=north] at ($(topLeft) + 5*(genUnit)$) {\subfigtextsize 0.3};
\node[anchor=north] at ($(topLeft) + 10*(genUnit)$) {\subfigtextsize 1.0};

% Interval endpoints
\draw[thin,color=black!50,dashed] ($(topLeft) + 5*(genUnit)$) -- +(0, 5.5);

% Axis titles
\node (topLabel) at ($(topLeft) + 5*(genUnit) + (0,-3.0)$) {$\subfigtextsize\textrm{Genome position}$};
\end{tikzpicture}
\end{subfigure}\\
\begin{subfigure}{\columnwidth}
\centering
...
Note the original node IDs have been retained for clarity.}
\end{figure}
...
\lambda^{\text{Dirac}}_{b; k_1, \ldots, k_r;s } &=
    \frac{c \psi^2 / 4}{1 + c \psi^2 / 4}
    \frac{4}{ \psi^2}\sum_{\ell = 0}^{s \wedge (4-r)} \binom{s}{\ell}
